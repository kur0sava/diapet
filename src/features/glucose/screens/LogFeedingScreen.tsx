...
export default function LogFeedingScreen() {
  ...
  const FOOD_TYPE_OPTIONS = useMemo(() => [
    ...
    { value: 'natural', label: t('feeding.natural', { defaultValue: i18n.language === 'ru' ? 'Натуральный' : 'Natural' }) },
  ...
  const disableGuard = useUnsavedChangesGuard(!!amount || !!notes || dateChanged || foodType !== 'dry' || !!selectedFood);
  ...
  const manualResult = useMemo(() => {
    ...
    return calculateDryMatter({ protein: p, fat: f, fiber: fi, ash: a, moisture: m });
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
          ...
          {selectedFood && selectedFood.carbsDM !== undefined && (
            <Card style={styles.nutritionCard}>
              <View style={styles.nutritionHeader}>
                ...
                <View style={[styles.verdictBadge, { backgroundColor: `${verdictColor(selectedFood.verdict)}20` }]}>
          ...
          {showManualNutrition && (
            <Card style={styles.nutritionCard}>
              ...
              >
                <Text style={[styles.nutritionTitle, { color: theme.colors.text }]}>
                  {t('feeding.manualNutrition', { defaultValue: i18n.language === 'ru' ? 'Ввести состав' : 'Enter nutrition' })}
                </Text>
              </TouchableOpacity>
              <View style={styles.macroGrid}>
                {[
                  { label: i18n.language === 'ru' ? 'Белок %' : 'Protein %', value: protein, setter: setProtein },
                  { label: i18n.language === 'ru' ? 'Жир %' : 'Fat %', value: fat, setter: setFat },
                  { label: i18n.language === 'ru' ? 'Клетчатка %' : 'Fiber %', value: fiber, setter: setFiber },
                  { label: i18n.language === 'ru' ? 'Зола %' : 'Ash %', value: ash, setter: setAsh },
                  { label: i18n.language === 'ru' ? 'Влага %' : 'Moisture %', value: moisture, setter: setMoisture },
                ].map(field => (
                  <View key={field.label} style={styles.macroField}>
                    <Text style={[styles.macroLabel, { color: theme.colors.textSecondary }]}>{field.label}</Text>
                    <Input
                      value={field.value}
              ...
              {manualResult && (
                <View style={[styles.resultRow, { borderTopColor: theme.colors.border }]}>
                  <View style={[styles.verdictBadge, { backgroundColor: `${verdictColor(manualResult.verdict)}20` }]}>
                    <Text style={[styles.verdictText, { color: verdictColor(manualResult.verdict) }]}>
                      {verdictLabel(manualResult.verdict)}
                    </Text>
                  </View>
                  <Text style={[styles.carbsValue, { color: verdictColor(manualResult.verdict) }]}>
                    {manualResult.carbsDM.toFixed(1)}% {i18n.language === 'ru' ? 'угл. СВ' : 'carbs DM'}
                  </Text>
                </View>
              )}
...
const styles = StyleSheet.create({
  ...
  selectFoodBtn: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 14, borderRadius: 12, borderWidth: 1 },
  selectFoodText: { flex: 1, fontSize: 14, fontWeight: '600' },
  nutritionCard: { paddingVertical: 16 },
  nutritionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  ...
  warningRow: { flexDirection: 'row', alignItems: 'center', gap: 6, padding: 10, borderRadius: 8, marginTop: 8 },
  ...
  resultRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingTop: 12, marginTop: 8, borderTopWidth: 0.5 },
