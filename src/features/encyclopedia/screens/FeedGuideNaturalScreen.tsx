...
export default function FeedGuideNaturalScreen() {
  ...
  const sections = useMemo(() => {
    const s: { key: string; title: string; data: Array<{ type: string; food?: NaturalFood }> }[] = [
      ...
      {
        key: 'supplements',
        title: t('feedGuide.natural.supplements'),
        data: [{ type: 'supplements' }],
      },
      {
        key: 'transition',
        title: t('feedGuide.natural.transitionTips'),
      ...
      return (
        ...
        >
          <View style={styles.foodHeader}>
            <View style={[styles.foodIconCircle, { backgroundColor: theme.colors.primary + '12' }]}>
              <Icon name={CATEGORY_ICONS[food.category]} size={20} color={theme.colors.primary} />
            </View>
            <View style={styles.foodInfo}>
              ...
              <View style={styles.badgeRow}>
                ...
                <View
                  style={[styles.freqBadge, { backgroundColor: theme.colors.surfaceSecondary }]}
                >
          ...
          <View style={styles.macroRow}>
            <View style={styles.macroItem}>
              ...
              <Text style={[styles.macroLabel, { color: theme.colors.textTertiary }]}>
                {t('feedGuide.natural.protein')}
              </Text>
            </View>
            <View style={styles.macroItem}>
              <Text style={[styles.macroValue, { color: theme.colors.text }]}>
  ...
  const renderItem = useCallback(
    ({ item }: { item: { type: string; food?: NaturalFood } }) => {
      if (item.type === 'info') {
        return (
          ...
          >
            {guide.supplements.map((s, i) => (
              <View key={i} style={styles.suppRow}>
                ...
                <Text style={[styles.suppDesc, { color: theme.colors.textSecondary }]}>
                  {s.desc[lang]}
                </Text>
              </View>
            ))}
      ...
      if (item.type === 'transition') {
        return (
          ...
          >
            <Text style={[styles.transText, { color: theme.colors.text }]}>
              {guide.transitionTips[lang]}
            </Text>
          </View>
        );
      }
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <SectionList
        ...
        keyExtractor={(item, index) =>
          item.type === 'food' && item.food ? item.food.id : `${item.type}-${index}`
        }
